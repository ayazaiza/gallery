...
/*fun String.hasMediaType(isVideo: Boolean) {
    val file = File(this)
    val lists = file.listFiles()?.toMutableList()
    if (lists != null) {
        lists.sortBy { it.isDirectory }
        for (list in lists) {
            if (list.isDirectory) {
                continue
            }
...
fun String.toFirstCap(): String {
    ...
    return buildString {
        append(takeFirst)
        append(this@toFirstCap.substring(1))
    }
}

fun String.shouldFolderBeVisibleForVideos(
    ...
    if (excludedPaths.contains(this) || excludedPaths.any { equals(it, true) })
        return false

    if (includedPaths.contains(this) || includedPaths.any { equals(it, true) } ||
        includedPaths.any { "$this/".startsWith("$it/", true) })
        return true

    val containNoMedia = if (showHidden) {
        false
    } else {
        folderNoMediaStatuses.getOrElse("$this/$NOMEDIA") { false } || ((!isRPlus() || Environment.isExternalStorageManager()) && File(
            this,
            NOMEDIA
        ).exists())
    }

    if (!showHidden && containNoMedia) {
        return false
    }
            ...
            for (i in 0 until count { it == '/' } - 1) {
                ...
                if (folderNoMediaStatuses.contains(pathToCheck)) {
                    if (folderNoMediaStatuses[pathToCheck] == true) {
                        ...
                        val noMediaExist = folderNoMediaStatuses.getOrElse(pathToCheck) { false } ||
                                File(pathToCheck).exists()
                        callback(pathToCheck, noMediaExist)
                        if (noMediaExist) {
                            containsNoMediaOrDot = true
                            break
                        }
            ...
            run repeatBlock@{
                repeat(count { it == '/' } - 1) {
                    ...
                    if (folderNoMediaStatuses.contains(pathToCheck)) {
                        ...
                        val noMediaExist =
                            folderNoMediaStatuses.getOrElse(pathToCheck) { false } || File(
                                pathToCheck
                            ).exists()
                        callback(pathToCheck, noMediaExist)
                        if (noMediaExist) {
...
fun String.shouldFolderBeVisibleOld(
    excludedPaths: MutableSet<String>,
    includedPaths: MutableSet<String>,
    showHidden: Boolean,
    folderNoMediaStatuses: HashMap<String, Boolean>,
    ...
    return if (!showHidden && containsNoMedia) {
        ...
        var containNoMediaOrDot = containsNoMedia || contains("/.")
        if (!containNoMediaOrDot) {
            var curPath = this
            for (i in 0 until count { it == '/' } - 1) {
                curPath = curPath.substringBeforeLast('/')
                val pathToCheck = "$curPath/$NOMEDIA"
...
fun String.stringForTime(): String {
    return run {
        val totalSeconds = (toLong() / 1000).toInt()
        ...
        if (size > 0) {
            ...
            when {
                ...
                k > 1 -> {
                    ...
                    dec.format(size.toDouble()) + " Bytes"
                }
            }
        } else {
            "00"
